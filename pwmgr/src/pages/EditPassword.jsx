...
import { FiLock, FiUser, FiKey, FiEye, FiEyeOff } from 'react-icons/fi';
import usePasswordStore from '../store/passwordStore';
import { calculateStrength, getStrengthLevel, getStrengthColor } from '../utils/crypto';
import Input from '../components/Input';
import Button from '../components/Button';

// Edit password page component
const EditPassword = () => {
  const navigate = useNavigate();
  const selectedPass = usePasswordStore(state => state.selectedPass);
  const updatePass = usePasswordStore(state => state.updatePass);
  const getDecryptedPass = usePasswordStore(state => state.getDecryptedPass);
  const clearSelection = usePasswordStore(state => state.clearSelection);
  ...
  useEffect(() => {
    ...
    // Load selected password data
    const decryptedPass = getDecryptedPass(selectedPass.id);
    setFormData({
      title: selectedPass.title,
      user: selectedPass.user,
      pass: decryptedPass,
      key: selectedPass.key,
    });
    setStrength(selectedPass.level);
  }, [selectedPass, navigate]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updatePass(selectedPass.id, formData);
    clearSelection();
    navigate('/passwords');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      >
        <h1 className="text-3xl font-bold text-dark-text mb-2">Edit Password</h1>
        <p className="text-dark-muted mb-8">Update password details</p>
      </motion.div>
